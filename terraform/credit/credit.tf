...
resource "okta_group" "credit_group" {
  name = "${var.udp_subdomain} ${var.demo_app_name} Group"
}
resource "okta_group_rule" "credit_group_rule" {
  name              = "${var.udp_subdomain} ${var.demo_app_name} Rule"
  status            = "ACTIVE"
  group_assignments = ["${okta_group.credit_group.id}"]
...
resource "okta_app_oauth" "credit" {
  ...
  groups         = ["${okta_group.credit_group.id}"]
}
