...
@is_authenticated
def dealer_profile():
    return render_template(get_app_vertical()+"/profile.html", templatename=get_app_vertical(), user_info=get_userinfo(), config=session[SESSION_INSTANCE_SETTINGS_KEY], _scheme="https")
...
def dealer_myapps():
    ...
    return render_template(get_app_vertical()+"/myapps.html", templatename=get_app_vertical(),user_info=user_info, config=session[SESSION_INSTANCE_SETTINGS_KEY],location=location, apps = get_apps_response,  _scheme="https")
...
def workflow_approvals():
    ...
    if request.method == "GET":
        ...
        if admin_group_id :
            ...
            return render_template(get_app_vertical()+"/workflow-approvals.html", templatename=get_app_vertical(),workflow_list=workflow_list, user_info=user_info,config=session[SESSION_INSTANCE_SETTINGS_KEY],_scheme="https")
        else:
            return render_template(get_app_vertical()+"/workflow-approvals.html", templatename=get_app_vertical(),error="Must be an Administrator of a dealership to approve requests", user_info=user_info,config=session[SESSION_INSTANCE_SETTINGS_KEY],_scheme="https")
...
def workflow_requests():
    ...
    if request.method == "GET":
        ...
        if is_user_dealership:
            ...
            return render_template(get_app_vertical()+"/workflow-requests.html", templatename=get_app_vertical(),user_info=user_info,workflow_list=workflow_list, config=session[SESSION_INSTANCE_SETTINGS_KEY],_scheme="https")
        else: #If not a user of a dealership, cannot request access to applications
            return render_template(get_app_vertical()+"/workflow-requests.html", templatename=get_app_vertical(),user_info=user_info,error="You have not been assigned to a dealership. Only users of a dealership can request access to applications", config=session[SESSION_INSTANCE_SETTINGS_KEY],_scheme="https")
