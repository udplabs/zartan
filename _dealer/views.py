...
def dealer_profile():
    ...
    return render_template(templatename+"/profile.html", templatename=templatename, oidc=oidc, user_info=user_info, config=default_settings)
...
def dealer_registration():
    ...
    try:
        ...
        if request.method == "GET":
            return render_template(templatename+"/registration.html", templatename=templatename,config=default_settings,user_data=user_data,setup_options=setup_options)
        ...
        return render_template(templatename+"/registration.html", templatename=templatename,config=default_settings, error = e, user_data=user_data,setup_options=setup_options) 
        
    return render_template(templatename+"/registration-completion.html", templatename=templatename,config=default_settings, email=request.form.get('email'))
...
def dealer_registration_state(stateToken):
    ...
    return render_template(templatename+"/registration-state.html", templatename=templatename,config=default_settings)
    
@dealer_views_bp.route("/registration-completion", methods=["GET"])
def dealer_registration_completion():
    return render_template(templatename+"/registration-completion.html", templatename=templatename,config=default_settings) 